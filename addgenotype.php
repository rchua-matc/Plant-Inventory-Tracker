<?php
    ...
    {
        // Get the genotype information from the form
        ...
        if (!empty($name))
        {
            ...
            {
                // Add the genotype to the DB
                $new_genotype = new Genotype();

                $new_genotype->setName($name);
                $new_genotype->setCategory($category);
                $new_genotype->setCalcium($calcium);
                $new_genotype->setTransgenic($transgenic);
                $new_genotype->setNotes($notes);

                $new_genotype->addGenotype($dbc);

                echo "<p>Genotype $name successfully added.</p>";
            ...
            {
                echo '<p>The calcium concentration must be numeric.</p>';
            }

        }
        else
        {
            echo '<p>The genotype name cannot be blank.</p>';
        }
    }
    else
    {
...
<h1 class="mt-3">Add A Genotype</h1>
<form action="<?= $_SERVER['PHP_SELF']; ?>" method="POST" class="mx-auto">
    ...
    <input type="submit" name="submit" value="Add Genotype" class="mt-3">
</form>
</div>